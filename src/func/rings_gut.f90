...
FUNCTION checkShortCut(rr) RESULT(no_short_cut)
    IMPLICIT NONE
    ! IN:
    integer, intent(in) :: rr(:)
    ! OUT:
    logical :: no_short_cut
    ! PRIVATE:
...
function add_ring(branch1, branch2) result(add_success)
    IMPLICIT NONE
    ! IN:
...
subroutine print_ringno(ring_l)
    ...
    maxn = maxval(ring_l)
    print *, info//' Maximum ring length is: ', maxn
!     if (maxn > 20) maxn = 20
